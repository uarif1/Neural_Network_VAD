...
def melbankm(p, n, fs, fl=0, fh=0.5):
  ...
  '''
    ...
    r = np.concatenate((1+fp[:k3], fp[k2-1, k4]))  # filter number_1
    # FFT bin_1 - b1
    c = np.concatenate((np.arange(1, k3+1), np.arange(k2, k4+1)))
    v = np.concatenate((pm[:k3], 1-pm[k2:k4]))
